...
cam_2_frame_rotation = np.array(
    [[0.7071068, -0.7071068, 0.000], [0.7071068, 0.7071068, 0], [0, 0, 1]]
).T  # 45deg around Z
...
def test_2_cameras_base_pose():
    ...
    cam_params_1 = CameraParameters(
        res_x=cam_1_resx, res_y=cam_1_resy, fov=cam_1_fov, R=R1, T=T1, z_near=cam_1_znear, z_far=cam_1_zfar
    )

    R2, T2 = get_torch3d_R_T(cam_2_frame_rotation, cam_2_pos)
    cam_params_2 = CameraParameters(
        res_x=cam_2_resx, res_y=cam_2_resy, fov=cam_2_fov, R=R2, T=T2, z_near=cam_1_znear, z_far=cam_1_zfar
    )

    masks = get_mesh_segmentation_batch(
        mesh_path=mesh1_path, scale=scale, cameras_parameters=[cam_params_1, cam_params_2], device="cpu"
    )

    cam_1_sim = CameraSimulator(
        resolution=(cam_1_resx, cam_1_resy), fovy=cam_1_fov, launch_viewer=False, world_file="./data/world_mug.xml"
    )
    cam_2_sim = CameraSimulator(
        resolution=(cam_2_resx, cam_2_resy), fovy=cam_2_fov, launch_viewer=False, world_file="./data/world_mug.xml"
    )
...
def test_1_camera_2_poses():
    ...
    cam_params_3 = CameraParameters(
        res_x=cam_3_resx, res_y=cam_3_resy, fov=cam_3_fov, R=R3, T=T3, z_near=cam_3_znear, z_far=cam_3_zfar
    )

    cam_3_sim = CameraSimulator(
        resolution=(cam_3_resx, cam_3_resy), fovy=cam_3_fov, launch_viewer=False, world_file="./data/world_mug.xml"
    )
    ...
    mask_pose1 = get_mesh_segmentation_batch(
        mesh_path=mesh1_path,
        scale=scale,
        cameras_parameters=cam_params_3,
        position=obj_position_1,
        orientation=obj_orientation_1,
        device="cpu",
    )
    ...
    mask_pose2 = get_mesh_segmentation_batch(
        mesh_path=mesh1_path,
        scale=scale,
        cameras_parameters=cam_params_3,
        position=obj_position_2,
        orientation=obj_orientation_2,
        device="cpu",
    )

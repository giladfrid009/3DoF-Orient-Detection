...
from utils import (
    ...
    plot_grid_masked_depth_on_image,
    compute_depth_squared_distance_in_intersection,
    masked_depth_diff,
)
...
if __name__ == "__main__":
    ...
    for obj_position, obj_orientation in zip(obj_positions, obj_orientations):
        ...
        cam_params = CameraParameters(
            res_x=cam_resx, res_y=cam_resy, fov=cam_fov, R=R, T=T, z_near=cam_znear, z_far=cam_zfar
        )
        mask, depth = get_mesh_segmentation_batch(
            mesh_path=mesh,
            scale=scale,
            cameras_parameters=cam_params,
    ...
    plot_grid_masked_depth_on_image(
        images, depth_pred, masks_pred, (1, 4), mask_alpha=0.99, title="Depth map masked by segmentation"
    )
    ...
    depth_distance = compute_depth_squared_distance_in_intersection(
        depth_actual_torch, depth_pred, mask_sam_torch, masks_pred_batch
    )
    ...
    fig, axs = plt.subplots(2, 4, figsize=(20, 10))
    for intr, depth_diff, ax_intr, ax_depth, iou_score, depth_dist in zip(
        intersection, masked_depth_diff, axs[0], axs[1], iou, depth_distance
    ):
        ax_intr.imshow(intr)
        ax_intr.axis("off")

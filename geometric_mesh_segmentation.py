...
def get_cameras(cameras_parameters: Union[List[CameraParameters], CameraParameters], device="auto"):
    ...
    cameras = FoVPerspectiveCameras(
        znear=z_near, zfar=z_far, aspect_ratio=aspect_ratio, fov=fov, R=R, T=T, device=device
    )
    return cameras, res_x, res_y


def get_mesh_segmentation_batch(
    mesh_path,
    cameras_parameters: Union[List[CameraParameters], CameraParameters],
    scale=1,
    position=torch.Tensor([0, 0, 0]),
    orientation=torch.Tensor([0, 0, 0]),
    ...
    raster_settings = RasterizationSettings(
        image_size=(int(res_x[0]), int(res_y[0])), blur_radius=np.log(1.0 / 1e-4 - 1.0) * sigma, faces_per_pixel=1
    )

...
def plot_grid_segmentation_masks(
    images, masks, row_cols=(1, 4), mask_alpha=0.8, title="", color=np.array([255, 30, 30])
):
...
def plot_grid_masked_depth_on_image(images, depth_images, masks, row_cols=(1, 4), mask_alpha=0.9, title=""):
    ...
    for im, depth_im, mask, ax in zip(images, depth_images, masks, axs):
        ...
        im_with_depth[mask.squeeze() != 0] = red_mask[mask.squeeze() != 0] * mask_alpha + (
            im[mask.squeeze() != 0] / 255
        ) * (1 - mask_alpha)
